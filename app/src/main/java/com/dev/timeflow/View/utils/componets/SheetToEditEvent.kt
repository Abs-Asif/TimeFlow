...
fun SheetToEditEvent(
    ...
    var eventName by remember(event.id) { mutableStateOf(event.name) }
    var startDate by remember(event.id) { mutableStateOf(event.startDate.toLocalDate()) }
    var endDate by remember(event.id) { mutableStateOf(event.endDate.toLocalDate()) }
